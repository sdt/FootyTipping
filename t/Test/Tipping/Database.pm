...
sub install {
    ...
    given ($driver) {
        ...
        when ('Pg') {
            use Test::postgresql ();
            $config->{test_database} = Test::postgresql->new()
                or croak $Test::postgresql::errstr;

            # STFU please postgres...
            my $dbh = DBI->connect($config->{test_database}->dsn);
        ...
        when ('mysql') {
            use Test::mysqld ();
            $config->{test_database} = Test::mysqld->new(
                    my_cnf => { 'skip-networking' => '' }
                    ) or croak $Test::mysqld::errstr;
        }

        when ('SQLite') {
            $dsn = q(dbi:SQLite:dbname=:memory:);
        }
    ...
    $config->{database}->{dsn} = $dsn // $config->{test_database}->dsn;
